...
const Tabs = ({ active, purchases, products, customerId, treatments }) => {
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex flex-col items-center">
            <h2 className="mb-2">
              {" "}
              Treatment History And Upcoming Treatments{" "}
            </h2>
            <ul>
              {treatments.map((treatment) => {
                return (
                  <li key={treatment._id}>
                    {treatment.treatmentId.treatmentName} on{" "}
                    {new Date(treatment.start).toLocaleDateString("en-UK")}
                  </li>
                );
              })}
            </ul>
